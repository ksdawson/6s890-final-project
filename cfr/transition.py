...
class Transition:
    ...
    def next_state(self, t, s, a1, a2):
        ...
        if time_idx == 1:
            ...
            for fut_t, assigned_dict in s.assignments.items():
                for (player, zone_id), count in assigned_dict.items():
        ...
        if time_idx in s.assignments:
            for (player, zone_id), count in s.assignments[time_idx].items():
                idx = zone_id - 1
                if player == 1:
                    new_s1[idx] += count
                else:
        ...
        # Update history
        next_assignments = {
            t: v.copy() for t, v in s.assignments.items() if t > time_idx
        }
        for future_t, player, zone_id, cnt in assignments_log:
            if future_t not in next_assignments:
                next_assignments[future_t] = {}
            key = (player, zone_id)
            next_assignments[future_t][key] = next_assignments[future_t].get(key, 0) + 1

        # Return the probability in normal space (exp)
        final_prob = math.exp(total_log_prob)

        # Convert to zero sum reward
        r1 = reward[1] - reward[2]
        r2 = reward[2] - reward[1]

        return tuple(new_s1), tuple(new_s2), r1, r2, final_prob, next_assignments
